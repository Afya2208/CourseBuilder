...
import type { Correlation, Task, TaskAnswer, UserAnswer } from '@/models/main.ts'
import api from '@/services/api.ts'
import { useUserStore } from '@/stores/user.ts'
import { indexToChar } from '@/util/methods.ts'
...
const checkTask = async () => {
	switch (theTask.value.taskTypeId) {
		...
		case 2: {
			if (user.value) {
				await api
					.post(`tasks/${theTask.value.id}/save-answer`, {
						...userAnswer.value,
		...
		case 3: {
			...
			for (let i = 0; i < mixedCorrelations.value.length; i++) {
				const userPut = userIndexCorrelations.value[i] // например, b) = index = 1
				if (userPut) {
					const index0 = 'a'.charCodeAt(0)
					const userIndex = userPut.charCodeAt(0) - index0 // b - a == 1
					const selectedId = mixedCorrelations.value[userIndex].rightId // по индексу ищем ответ
					const leftId = mixedCorrelations.value[i]?.id
		...
		case 4: {
			const selectedAnswer = answerOptions.value.find(
		...
		case 5: {
			...
			for (const answer of answerOptions.value) {
				const didUserSelectIt = userAnswer.value.selectedTaskAnswersId.includes(answer.id)
...
defineExpose({ checkTask, update, userAnswer, theTask })
	...
	<div>
		<div v-if="props.isEditing" class="control_div">
			...
			<button
				...
				:disabled="index === 0"
				@click="emit('upClicked')"
			>
				⬆️
			</button>
			<button
				class="btn btn-outline-dark btn-sm m-1"
				:disabled="index === tasksCount - 1"
				@click="emit('downClicked')"
			>
				⬇️
			</button>
		</div>
		<h5
			:class="{
				'question-p-correct': userAnswer.isRight == true,
				'question-p-bad': userAnswer.isRight == false,
			}"
		>
			Задание {{ props.index + 1 }}: {{ theTask.question }}
		</h5>
					...
					<tr v-for="(correlation, index) in mixedCorrelations">
						...
						<td>
							<select class="form-control-sm" v-model="userIndexCorrelations[index]">
								<option :value="opt" v-for="opt in correlationOptionsIndexes">
									{{ opt }}
								</option>
							</select>
						</td>
